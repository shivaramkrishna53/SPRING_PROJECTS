...
public class SpringBatchProj18ReadingFromDbAndWritingToJsonFileApplication {
	...
	{
		return jobbuilderfac.get("readingDbAndWritingToJsonUsingCustomProcessorJob").start(readingDbAndWritingToJsonStep()).build();
	}

	@Bean
	public Step readingDbAndWritingToJsonStep() {
		// TODO Auto-generated method stub
		return stepbuilderfac.get("readingDbAndWritingToJsonStep").<Order,TransformedOrder>chunk(10).reader(dbreader()).processor(dotheprocessing()).writer(jsonwriter()).build();
	}
	...
	public ItemWriter<TransformedOrder> jsonwriter() {
		...
		return new JsonFileItemWriterBuilder<TransformedOrder>().append(true).resource(new FileSystemResource("E:\\shiva java developer full stack\\springprojects\\SpringBatchProj-20-CustomItemProcessorDemo-ReadingFromDbAndWritingToJsonFile\\src\\main\\java\\com\\tcs\\result\\abcd.json")).jsonObjectMarshaller(new JacksonJsonObjectMarshaller<TransformedOrder>()).name("jsonitmwriterewew").build();
	}
