...
public class SpringBatchProj18ReadingFromDbAndWritingToJsonFileApplication {
	...
	public Step readingDbAndWritingToJsonStep() {
		...
		return stepbuilderfac.get("readingDbAndWritingToJsonStep").<Order,Order>chunk(10).reader(dbreader()).processor(dotheprocessing()).writer(jsonwriter()).build();
	}

	@Bean
	public ItemProcessor<Order,Order> dotheprocessing() {
		// TODO Auto-generated method stub
		BeanValidatingItemProcessor<Order> itemprocessor=new BeanValidatingItemProcessor<Order>();
		itemprocessor.setFilter(true);     //check for order.java also filters applied over email field in order.java file
	...
	public ItemWriter<Order> jsonwriter() {
		...
		return new JsonFileItemWriterBuilder<Order>().append(true).resource(new FileSystemResource("E:\\shiva java developer full stack\\springprojects\\SpringBatchProj-19-ItemProcessorDemo-ReadingFromDbAndWritingToJsonFile2\\src\\main\\java\\com\\tcs\\result\\abcd.json")).jsonObjectMarshaller(new JacksonJsonObjectMarshaller<Order>()).name("jsonitmwriterewew").build();
	}
