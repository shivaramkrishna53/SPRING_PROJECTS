...
public class SpringBatchProj18ReadingFromDbAndWritingToJsonFileApplication {
	...
	public Step readingDbAndWritingToJsonStep() {
		...
		return stepbuilderfac.get("readingDbAndWritingToJsonStep").<Order,Order>chunk(100).reader(dbreader()).processor(dotheprocessing()).writer(jsonwriter()).build();
	}
	...
	public ItemWriter<Order> jsonwriter() {
		...
		return new JsonFileItemWriterBuilder<Order>().append(true).resource(new FileSystemResource("E:\\shiva java developer full stack\\springprojects\\SpringBatchProj-19-ItemProcessorDemo-ReadingFromDbAndWritingToJsonFile2\\src\\main\\java\\com\\tcs\\result\\abcd.json")).jsonObjectMarshaller(new JacksonJsonObjectMarshaller<Order>()).name("jsonitmwriterew").build();
	}
