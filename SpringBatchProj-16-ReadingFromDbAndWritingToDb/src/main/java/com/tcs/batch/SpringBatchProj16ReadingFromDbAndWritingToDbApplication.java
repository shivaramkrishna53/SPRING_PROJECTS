...
public class SpringBatchProj16ReadingFromDbAndWritingToDbApplication {
	...
	public static final String writing_query="insert into shipped_order_output(order_id,first_name,last_name,email,cost,item_id,item_name,ship_date) values(?,?,?,?,?,?,?,?)";
	...
	public ItemWriter<Order> writetoanotherDb() {
		...
		return new JdbcBatchItemWriterBuilder().dataSource(ds).sql(writing_query).itemPreparedStatementSetter(new ItemPreparedStatementSetter<Order>() {
			...
			public void setValues(Order item, PreparedStatement ps) throws SQLException {
				...
				ps.setDate(8,(Date) item.getShipDate());
